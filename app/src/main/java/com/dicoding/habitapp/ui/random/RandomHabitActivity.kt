...
class RandomHabitActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.priorityLevelHigh.observe(this) {
            if (it == null) else adapter.submitData(RandomHabitAdapter.PageType.HIGH, it)
        }
        viewModel.priorityLevelMedium.observe(this) {
            if (it == null) else adapter.submitData(RandomHabitAdapter.PageType.MEDIUM, it)
        }
        viewModel.priorityLevelLow.observe(this) {
            if (it == null) else adapter.submitData(RandomHabitAdapter.PageType.LOW, it)
        }
